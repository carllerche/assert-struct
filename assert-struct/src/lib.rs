...
// Additional implementations for common patterns
#[cfg(feature = "regex")]
mod additional_like_impls {
    use super::Like;

    // Support for matching against multiple patterns (OR logic)
    impl<T> Like<Vec<T>> for String
    where
        String: Like<T>,
    {
        /// Returns true if the string matches ANY of the patterns in the vector
        fn like(&self, patterns: &Vec<T>) -> bool {
            patterns.iter().any(|pattern| self.like(pattern))
        }
    }

    impl<T> Like<Vec<T>> for &str
    where
        for<'a> &'a str: Like<T>,
    {
        /// Returns true if the string matches ANY of the patterns in the vector
    ...
    // Support for Option patterns - useful for optional matching
    impl<T, P> Like<Option<P>> for T
    where
        T: Like<P>,
    {
        /// None always matches (acts as wildcard), Some(pattern) uses the pattern
        fn like(&self, pattern: &Option<P>) -> bool {
            match pattern {
                None => true, // None acts as a wildcard - matches anything
                Some(p) => self.like(p),
            }
        }
    }

    // Support for matching with closures/functions
    impl<T> Like<Box<dyn Fn(&T) -> bool>> for T {
        /// Allows using boxed closures as patterns
        fn like(&self, predicate: &Box<dyn Fn(&T) -> bool>) -> bool {
            predicate(self)
        }
    }

    // Support for AND logic with tuple of patterns
    impl<T, P1, P2> Like<(P1, P2)> for T
    where
        T: Like<P1> + Like<P2>,
    {
        /// Returns true if the value matches BOTH patterns
        fn like(&self, patterns: &(P1, P2)) -> bool {
            self.like(&patterns.0) && self.like(&patterns.1)
        }
    }
    ...
    impl Like<CaseInsensitive> for String {
        fn like(&self, pattern: &CaseInsensitive) -> bool {
            self.to_lowercase() == pattern.0.to_lowercase()
        }
    }

    impl Like<CaseInsensitive> for &str {
        fn like(&self, pattern: &CaseInsensitive) -> bool {
            self.to_lowercase() == pattern.0.to_lowercase()
        }
    }

    // Support for prefix/suffix matching
    #[derive(Debug, Clone)]
    pub struct Prefix(pub String);
    #[derive(Debug, Clone)]
    pub struct Suffix(pub String);

    impl Like<Prefix> for String {
        fn like(&self, pattern: &Prefix) -> bool {
            self.starts_with(&pattern.0)
        }
    }

    impl Like<Prefix> for &str {
        fn like(&self, pattern: &Prefix) -> bool {
            self.starts_with(&pattern.0)
        }
    }

    impl Like<Suffix> for String {
        fn like(&self, pattern: &Suffix) -> bool {
            self.ends_with(&pattern.0)
        }
    }

    impl Like<Suffix> for &str {
        fn like(&self, pattern: &Suffix) -> bool {
            self.ends_with(&pattern.0)
        }
    }
...
// Re-export the utility types
#[cfg(feature = "regex")]
pub use additional_like_impls::{CaseInsensitive, Prefix, Suffix};
