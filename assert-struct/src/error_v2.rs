//! Two-pass error rendering system for assert-struct.
//!
//! This module implements a clean separation between:
//! - Pass 1: Building a structural representation of errors
//! - Pass 2: Rendering that structure to a string
//!
//! This separation allows for future enhancements like pretty-printing
//! without affecting the error detection and structure building logic.

use crate::error::{ErrorContext, ErrorType, PatternNode};
        ...
        if let Some((error_field_path, error_line, _actual_value, _error_type)) = error_data {
            ...
            let section = ErrorSection {
                ...
                location: ErrorLocation {
                    ...
                    line_number: error_line, // TODO: Fix line number source in PatternNode
                },
                fragment,
            };
...
/// Build the field path for a tuple (without the numeric index)
fn build_tuple_field_path(field_path: &[String]) -> String {
    // Remove the last component if it's a number (tuple index)
